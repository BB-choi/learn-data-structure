...
<p align=center><img src="https://user-images.githubusercontent.com/31379392/148685007-17f75f19-ac5c-4319-9408-a8f0e176ed7c.jpg" width = "600"><br>윤성우 저, 열혈 자료 구조</p>

- **최소 비용 신장 트리** `MST` (minimum cost spanning tree)
: 신장 트리의 모든 간선의 가중치 합이 최소인 그래프. '최소 신장 트리'라고도 함.

> 최소 비용 신장 트리의 구성에 사용되는 알고리즘  
> - 크루스칼(Kruskal) 알고리즘
...
- 오름차순 : 그래프에 간선을 하나씩 추가하는 방식  
**how to**  

> 1. 가중치를 기준으로 간선을 오름차순으로 정렬  
> 2. 낮은 가중치의 간선부터 하나씩 그래프에 추가  
>> 💡 사이클을 형성하는 간선은 추가하지 않음!  
> 3. 위 과정 반복
> 4. 간선의 수가 정점의 수보다 하나 적을 때 MST 완성
>> 간선의 수 + 1 = 정점의 수     // MST의 특징

- 내림차순 : 그래프의 간선을 하나씩 삭제하는 방식  
**how to**  
> 1. 가중치를 기준으로 간선을 내림차순으로 정렬  
> 2. 높은 가중치의 간선부터 시작해서 하나씩 그래프에서 제거  
>> 💡 두 정점을 연결하는 다른 경로가 없으면, 해당 간선은 제거하지 않음!  
> 3. 위 과정 반복  
> 4. 간선의 수가 정점의 수보다 하나 적을 때 MST 완성  // 간선의 수 + 1 = 정점의 수  
